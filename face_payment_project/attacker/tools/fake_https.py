#!/usr/bin/python3
# fake_https.py
# Simple Fake HTTPS Video Server listening directly on port 443
# Serves video from media/videos/latest.mp4 in a loop
# Requires sudo to run.
# Located in: facial-payment-attack/tools/
...
# --- Configuration ---
# Use relative paths assuming script is run from tools/ directory
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
CERT_DIR = os.path.join(BASE_DIR, "certs") # certs directory inside tools/
MEDIA_DIR = os.path.join(BASE_DIR, "media", "videos") # videos directory
LATEST_VIDEO = os.path.join(MEDIA_DIR, "latest.mp4") # path to latest video
LISTEN_HOST = '0.0.0.0'
LISTEN_PORT = 443 # Default HTTPS port, requires root
FRAME_WIDTH = 640
FRAME_HEIGHT = 480
FPS = 20 # Playback FPS
# --- End Configuration ---
...
def create_default_image():
    """创建默认的红色"INTERCEPTED"图像，当视频文件不可用时使用"""
...
def generate_ssl_cert():
    ...
    try:
        ...
        subj.C = "XX" # Country
        subj.ST = "State" # State
        subj.L = "City" # Locality
        subj.O = "Fake Org" # Org Name
        subj.OU = "Fake OU" # OU Name
        try:
             subj.CN = socket.gethostname() # Common Name
        except Exception:
        ...
        try:
            if video_available:
                ...
                if os.path.exists(LATEST_VIDEO):
                    ...
                    if time.time() - mod_time < 15:  # 如果文件在15秒内被修改过
