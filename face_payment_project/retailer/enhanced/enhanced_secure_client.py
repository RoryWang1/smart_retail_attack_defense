...
class ClientState:
    ...
    def __init__(self):
        ...
        self.log_status = {
            'cache_mode_logged': False,      # Cache mode log recorded
            'attack_logged': False,          # Attack detection log recorded
            'arp_spoof_logged': False,       # ARP spoofing log recorded
            'frame_validation_logged': False, # Frame validation failure log recorded
            'verification_failure_logged': False, # Signature verification failure log recorded
            'network_instability_logged': False   # Network instability log recorded
        }
...
class SecurityStateManager:
            ...
            if os.path.exists(config.security_state_file):
                with open(config.security_state_file, 'r') as f:
                    ...
                    if saved_state.get('attack_detected', False):
                        ...
                        logger.warning("Loaded previously detected attack state - Enabling enhanced defense mode")
                    ...
                    logger.info(f"Loaded security state from {config.security_state_file}")
    ...
    def save_security_state():
        ...
        try:
            ...
            logger.info(f"Security state saved to {config.security_state_file}")
        except Exception as e:
...
class NetworkSecurityManager:
    ...
    def flush_arp_table():
        ...
        try:
            logger.info("Actively flushing ARP table...")
            ...
            logger.info("ARP table flushed successfully")
            return True
        except Exception as e:
            logger.error(f"Error flushing ARP table: {e}")
        ...
        try:
            ...
            for ip, mac in current_mappings.items():
                if ip in state.trusted_ips and state.trusted_ips[ip] != mac:
                    # Log only once per IP change
                    logger.warning(f"ARP mapping change: IP {ip} changed from {state.trusted_ips[ip]} to {mac}")
            ...
            if suspicious_changes >= 2:
                ...
                logger.warning("Single ARP table change detected - Monitoring closely")
                ...
                # Reset ARP spoofing log status, allowing logging next time it's detected
                state.log_status['arp_spoof_logged'] = False
            
            return True
        
        except subprocess.TimeoutExpired:
            logger.error("ARP command execution timed out")
            return False
        except Exception as e:
            logger.error(f"ARP security check error: {e}")
            return False
        ...
        try:
            ...
            if host_ip in arp_entries:
                ...
                if 'server' in state.trusted_ips and state.trusted_ips['server'] != current_mac:
                    logger.warning(f"Server MAC address changed: Previous {state.trusted_ips['server']} -> Now {current_mac}")
                    state.security_status['attack_indicators'] += 2
                    return True
            ...
            logger.error(f"Error checking server ARP entry: {e}")
            return False
    ...
    def network_monitor_thread():
        ...
        while state.network_monitor_active and state.running:
            try:
                ...
                if current_time - last_check_time >= check_interval:
                    ...
                    if state.attack_detected or state.security_status['attack_indicators'] > 2:
                        ...
                        if state.server_host:
                            ...
                            if server_check:
                                # Log suspicious server ARP only once
                                if not state.log_status.get('server_arp_suspicious', False):
                                    logger.warning("Server ARP entry suspicious - Potentially tampered")
                ...
                logger.error(f"Network monitoring thread error: {e}")
...
class SecurityManager:
    ...
    def verify_server_certificate(url):
        ...
        try:
            logger.info(f"Verifying server certificate: {url}")
            ...
            if suspicious:
                logger.warning(f"Server IP's ARP entry is suspicious: {host}")
                if state.attack_detected:
                    logger.error("Previous attack detected and server ARP entry is suspicious - Rejecting connection")
            ...
            if os.path.exists(fingerprint_file):
                ...
                logger.info(f"Loaded saved server fingerprint: {saved_fingerprint}")
            
            # Get current certificate fingerprint
            current_fingerprint = SecurityManager.get_certificate_fingerprint(host, port)
            
            if not current_fingerprint:
                logger.error("Failed to get certificate fingerprint")
                state.security_status['ssl_valid'] = False
                return False
            ...
            if saved_fingerprint:
                if saved_fingerprint != current_fingerprint:
                    ...
                    logger.error(f"Saved: {saved_fingerprint}")
                    logger.error(f"Current: {current_fingerprint}")
                    ...
                    if state.attack_detected or state.security_status['attack_indicators'] > 0:
                        logger.error("Previous attack detected and certificate fingerprint mismatch - Rejecting connection")
                        state.security_status['ssl_valid'] = False
                        state.security_status['attack_indicators'] += 2
                        state.attack_detected = True
                    ...
                    if hasattr(args, 'trust_on_first_use') and args.trust_on_first_use:
                        logger.warning("Trust On First Use mode enabled, updating fingerprint")
                    ...
                    logger.info("Certificate fingerprint verification successful")
                    state.security_status['ssl_valid'] = True
                    return True
            else:
                # First connection, save fingerprint
                logger.info("First connection to server, saving certificate fingerprint")
                with open(fingerprint_file, 'w') as f:
                    f.write(current_fingerprint)
                state.security_status['ssl_valid'] = True
                return True
    ...
    def get_certificate_fingerprint(host, port):
        ...
        try:
            ...
            logger.error(f"Error getting certificate fingerprint: {e}")
            logger.error(traceback.format_exc())
            return None
    ...
    def respond_to_challenge(challenge_data):
        ...
        if not state.session_key:
            logger.error("Cannot respond to challenge: No session key")
            return None
    ...
    def strict_verify_frame_signature(frame_id, signature, frame_data=None):
        ...
        try:
            ...
            if not signature or len(signature) < 64:
                ...
                if state.verification_failure_count == 1 or state.verification_failure_count % 20 == 0:
                    if not state.log_status['verification_failure_logged']:
                        logger.warning(f"Frame signature verification failed: ID {frame_id}, incorrect signature length, consecutive failures: {state.verification_failure_count}")
                ...
                if state.verification_failure_count > config.verification_threshold:
                    ...
                    if state.verification_failure_count == config.verification_threshold + 1:
                        logger.error(f"Multiple signature verification failures ({state.verification_failure_count}), possible interception attack")
            ...
            if not frame_id or not isinstance(frame_id, str) or ":" not in frame_id:
                ...
                if state.verification_failure_count % 50 == 0:
                    logger.warning(f"Invalid frame ID format: {frame_id}, consecutive failures: {state.verification_failure_count}")
            ...
            try:
                ...
                if len(parts) >= 2:
                    ...
                    if 'last_frame_number' in state.frame_sequence_tracker:
                        ...
                        if frame_number > last_number + 100:
                            ...
                            logger.warning(f"Suspicious large frame number jump: {last_number} -> {frame_number}")
                        ...
                        # Detect frame number regression (possible replay attack)
                            ...
                            if not state.log_status.get('frame_regression_logged', False):
                                logger.warning(f"Suspicious frame number regression: {last_number} -> {frame_number}")
                                state.log_status['frame_regression_logged'] = True
                            
                            state.verification_failure_count += 1
                            state.security_status['attack_indicators'] += 0.5
                            if state.verification_failure_count > config.verification_threshold:
                                logger.error("Possible frame replay attack detected")
                                state.attack_detected = True
                                NetworkSecurityManager.flush_arp_table()
                            return False
                ...
                logger.debug(f"Non-standard frame sequence format: {frame_id}")
            ...
            if state.verification_failure_count > 0:
                ...
                if state.verification_failure_count >= 5:
                    logger.info(f"Frame verification recovered, resetting failure count ({state.verification_failure_count} -> 0)")
            ...
            logger.error(f"Error during frame signature verification: {e}")
            logger.error(traceback.format_exc())
            state.verification_failure_count += 1
            return False
    ...
    def process_server_challenge(challenge):
        ...
        try:
            ...
            if not challenge_id or not challenge_data:
                logger.warning("Invalid challenge format")
                return
            ...
            if not response:
                logger.warning("Failed to generate challenge response")
                return
    ...
    @staticmethod
    def get_timestamp():
        """Get formatted timestamp including milliseconds"""
    ...
    def enhanced_handle_frame_display(frame_data, window_name, last_frame):
        ...
        if (isinstance(frame_data, tuple) and 
            ...
            if not state.log_status['attack_logged']:
                logger.warning("Attack detected, ARP table refreshed")
        ...
        if state.attack_detected and not state.log_status['cache_mode_logged']:
            logger.warning("Attack detected, forcing cache mode")
        ...
        if len(frame_data) >= 4:
            ...
            if not is_valid:
                ...
                if not state.log_status['frame_validation_logged']:
                    logger.warning("Frame validation failed, using cache frame")
        ...
        for key, value in headers.items():
            if isinstance(value, str):
                try:
                    ...
                    logger.debug(f"Header value encoded: {key}={encoded_value[:10]}...")
            else:
                safe_headers[key] = value
        return safe_headers
    ...
    def authenticate_with_server(url):
        ...
        try:
            ...
            if not state.device_fingerprint:
                ...
                logger.info(f"Generated device fingerprint: {state.device_fingerprint[:16]}...")
                ...
                try:
                    ...
                    if result.get('status') == 'success':
                        ...
                        if server_info:
                            ...
                            logger.info(f"Server info: {state.server_info}")
                ...
                if current_time - state.last_heartbeat_time >= config.heartbeat_interval:
                    ...
                    if response.status_code == 200:
                        ...
                        if state.security_status['heartbeat_failures'] >= 3:
                            logger.error("Multiple heartbeat failures, invalidating session")
...
class VideoStreamManager:
    ...
    def fetch_frames_thread(url):
        ...
        logger.info(f"Starting video frame fetching thread: {url}")
        ...
        if (state.attack_detected or state.security_status['attack_indicators'] > 2 or 
            ...
            logger.warning("Detected previous attack state, resetting network environment...")
        ...
        if (state.attack_detected or state.security_status['attack_indicators'] > 2 or 
            state.security_status['arp_spoof_detected']):
            logger.warning("Attack detected, using enhanced security checks")
            ...
            if state.server_host:
                ...
                if not arp_check_result or server_check_result:
                    logger.error("Security check failed, possible ongoing attack")
        ...
        # Check if session token exists, if not, try to authenticate first
        if not state.session_token:
            logger.warning("No valid session token, please authenticate first")
            # Can attempt to re-initiate authentication request
            base_url = url.rsplit('/', 1)[0] if '/' in url else url
            auth_url = f"{base_url}/api/auth"
            try:
                logger.info(f"Attempting automatic re-authentication: {auth_url}")
                HttpManager.authenticate_with_server(auth_url)
            except Exception as e:
        ...
        # Ensure client ID header is set
        client_id = config.client_id
        if client_id:
            headers['X-Client-ID'] = client_id
        else:
            logger.error("No client ID, unable to establish secure connection")
            # Try adding error frame to queue
            error_message = "Client ID missing, unable to connect to secure video stream"
            error_frame = FrameManager.create_warning_frame(error_message, high_priority=True)
            state.frame_queue.put((error_frame, None, None, False))
            return
        ...
        while state.running:
            try:
                ...
                if consecutive_failures >= max_consecutive_failures:
                    logger.warning(f"Reached maximum consecutive failure count ({max_consecutive_failures})")
                ...
                logger.debug(f"Connecting to video stream: {url}")
                ...
                logger.info("Connected to HTTPS video stream")
                ...
                for chunk in response.iter_content(chunk_size=4096):  # Larger buffer
                    ...
                    if time.time() - last_frame_time > 10:  # Increased timeout threshold
                        logger.warning("10 seconds without complete frame, reconnecting")
                        break
                                ...
                                if len(jpeg_data) > 100:
                                    ...
                                    if img is not None and img.size > 0:
                                        ...
                                        if frames_received % 10 == 0:
                                            logger.info(f"Successfully received {frames_received} frames")
                ...
                # Log disconnect
                logger.info(f"Stream ended or connection lost, received {frames_received} frames")
                
            except (requests.exceptions.ReadTimeout, requests.exceptions.ConnectionError) as e:
                # Network-related errors
                error_type = "Connection timeout" if isinstance(e, requests.exceptions.ReadTimeout) else "Connection error"
                logger.error(f"Video stream error: {error_type}")
                ...
                if state.network_error_count >= config.max_network_errors:
                    if state.network_error_count > config.max_network_errors * 2:
                        ...
                        if not state.log_status['network_instability_logged']:
                            logger.warning(f"Detected network instability, error count {state.network_error_count}")
                            state.log_status['network_instability_logged'] = True
                    else:
                        logger.warning(f"Detected network error ({state.network_error_count}), closely monitoring")
    ...
    def monitor_video_stream(url):
        ...
        logger.info(f"Starting enhanced security video monitoring: {url}")
        ...
        if (state.attack_detected or state.security_status['attack_indicators'] > 2 or 
            ...
            logger.warning("Detected previous attack - automatically enabling SAFETY mode")
        ...
        while verification_attempts < max_verification_attempts and not verification_success:
            ...
            try:
                if SecurityManager.verify_server_certificate(url):
                    ...
                    logger.info("Server certificate verification successful")
                    break
        ...
        if not auth_success:
            logger.warning("Authentication failed, continuing with limited security features")
            cv2.imshow(window_name, FrameManager.create_warning_frame(
            ...
            while state.running:
                ...
                if current_time - last_ui_update_time > 0.05:  # 20fps refresh rate
                    ...
                    if should_use_cache and not state.attack_detected:
                        if not cache_mode_active:
                            logger.info("Switching to cache mode")
                            cache_mode_active = True
...
def main():
    ...
    logger.info("Refreshing ARP table at startup...")
    NetworkSecurityManager.flush_arp_table()
    time.sleep(1)  # Wait for ARP table refresh
    
    # Generate device fingerprint
    state.device_fingerprint = SecurityManager.generate_device_fingerprint()
    logger.info(f"Device fingerprint: {state.device_fingerprint[:16]}...")
    ...
    try:
        while reconnect:
            ...
            if reconnect and state.running:
                logger.info("Refreshing ARP table before reconnection...")
                NetworkSecurityManager.flush_arp_table()
                time.sleep(1)  # Wait for ARP table refresh
